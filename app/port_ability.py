...
#--------------------------------------------------------------------------------------
# port_ability.py      Modified: Monday, October 29, 2018 12:34 PM
#
#
# If Pythonized...
#
...
available_actions = ['test', 'stop', 'restart', 'backup', 'fix-permissions', 'pull-data', 'push-code', 'push-data']
...
def do_push_code(target, target_env):
  ...
  cmd = "rsync -aruvi {4}/{3}/. {0}@{1}:{2}/{3}/ --progress --exclude=mariadb-init/ --exclude=sites/{3}/files/ --exclude=.git".format(target_env['PROD_SERVER_USER'], target_env['PROD_SERVER_ADDRESS'], target_env['PROD_SERVER_STACKS'], target, target_env['STACKS'])
...
#--------------------------------
def do_push_data(target, target_env):
  global client, base_dir

  # Determine the target's Drupal version.  If None, there's no reason to push data.
  try:
    v = target_env['DRUPAL_VERSION']
  except:
    yellow("Target '{0}' has no DRUPAL_VERSION parameter so no push-data is possible".format(target))
    return
  ...
  # Build an rsync command to push database from the target directory to PROD...
  cmd = "rsync -aruvi {4}/{3}/mariadb-init/. {0}@{1}:{2}/{3}/mariadb-init/ --progress  --exclude=.git".format(target_env['PROD_SERVER_USER'], target_env['PROD_SERVER_ADDRESS'], target_env['PROD_SERVER_STACKS'], target, target_env['STACKS'])
  green("Pushing data via: '{0}'".format(cmd))
  try:
    # debug("Command '{0}' is disabled.".format(cmd))
    os.system(cmd)
  except:
  ...
  # Build an rsync command to push data files from the target directory to PROD...
  cmd = "rsync -aruvi {4}/{3}/html/web/sites/{3}/files/. {0}@{1}:{2}/{3}/html/web/sites/{3}/files/ --progress  --exclude=.git".format(target_env['PROD_SERVER_USER'], target_env['PROD_SERVER_ADDRESS'], target_env['PROD_SERVER_STACKS'], target, target_env['STACKS'])
  green("Pushing data via: '{0}'".format(cmd))
  try:
    # debug("Command '{0}' is disabled.".format(cmd))
    os.system(cmd)
...
if __name__ == "__main__":
  ...
  for target in args.targets:
    ...
    if args.action[0] == 'push-data':
      do_push_data(target, target_env)
